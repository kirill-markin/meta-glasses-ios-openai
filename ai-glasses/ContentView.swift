...
struct ContentView: View {
    ...
    var body: some View {
        Group {
            ...
            case .limited:
                // Limited status is not applicable to Bluetooth, treat as authorized
                MainAppView()
            }
        }
        .onAppear {
            permissionsManager.refreshBluetooth()
        }
        .onChange(of: scenePhase) {
            // Refresh Bluetooth status when returning from Settings
...
/// Main app view with tabs - only created after Bluetooth permission is granted
...
/// Pre-permission screen explaining why Bluetooth is required
private struct BluetoothPermissionView: View {
    ...
    var body: some View {
        VStack(spacing: 32) {
            ...
            // Description
            VStack(spacing: 16) {
                Text("This app needs Bluetooth to connect to your Meta Ray-Ban glasses.")
                    .multilineTextAlignment(.center)
                
                Text("Without Bluetooth access, the app cannot discover or communicate with your glasses.")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                    .multilineTextAlignment(.center)
            }
...
// MARK: - Bluetooth Denied View

/// Screen shown when Bluetooth access has been denied
private struct BluetoothDeniedView: View {
    ...
    var body: some View {
        VStack(spacing: 32) {
            ...
            // Description
            VStack(spacing: 16) {
                Text("Bluetooth access is required to connect to your Meta Ray-Ban glasses.")
                    .multilineTextAlignment(.center)
                
                Text("Please enable Bluetooth access in Settings to use this app.")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                    .multilineTextAlignment(.center)
            }
